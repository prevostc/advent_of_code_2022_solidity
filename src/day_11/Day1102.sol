...
contract Day1102 {
    ...
    function init(string memory input) public {
        ...
        commonFactor = 1;
        for (uint256 idx = 0; idx < monkeys.length; idx++) {
            Monkey memory m = monkeys[idx];
            commonFactor *= m.test.divisibleBy;
        }
    }
    ...
    function monkeyRound(uint256 idx) internal {
        ...
        for (uint256 i; i < m.items.length; i++) {
            ...
            // apply boredom
            // we need to keep the number "manageable"
            // so we want to reduce the size of the number while keeping every test result and operation the same
            // so we "wrap" the number around the common factor
            item = item % commonFactor;
